...
const Test = () => {
  ...
  // 1. DEFINE a placeholder 'pause' and 'resume' from useRef.
  //    This breaks the circular dependency between handleSubmit and useTimer.
  ...
  // 2. MOVE handleSubmit UP HERE.
  //    Now it's defined before anything tries to use it.
  const handleSubmit = useCallback(async (auto = false) => {
    if (!currentTest || isSubmitting) return;
    setIsSubmitting(true);

    timerControls.current.pause(); // Use the ref here
    ...
    const payload = {
      ...
      // IMPORTANT: Use the ref to get the final timeLeft value.
    ...
    try {
        ...
        if (!auto) timerControls.current.resume(); // Use the ref here
    }
  // Remove timeLeft, pause, and resume from dependencies as they come from the ref now.
  }, [answers, currentTest, exam, mockTest, dispatch, navigate, isSubmitting]);

  // 3. DEFINE handleTimeUp using the now-defined handleSubmit.
  ...
  // 5. UPDATE the ref with the actual timer functions.
  // This ensures handleSubmit has access to the latest versions.
  useEffect(() => {
